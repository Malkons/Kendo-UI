...
// this is the "show all cars" button, removes all filters
...
//this button shows the legend that is generated by kendo
...
$(document).ready(function () {
    ...
    function filterMake() {
        ...
        for (var i = 0; i < favorite.length; i++) {
            ...
            checkboxFilters.push({ field: 'make', operator: 'contains', value: favorite[i] });
        };

        rangesliderFilters.push({ field: 'modelyear', operator: 'gte', value: modelYearSlider.values()[0] });
        rangesliderFilters.push({ field: 'modelyear', operator: 'lte', value: modelYearSlider.values()[1] });
        rangesliderFilters.push({ field: 'acceleration', operator: 'gte', value: accelerationSlider.values()[0] });
        rangesliderFilters.push({ field: 'acceleration', operator: 'lte', value: accelerationSlider.values()[1] });
        rangesliderFilters.push({ field: 'weight', operator: 'gte', value: weightSlider.values()[0] });
        rangesliderFilters.push({ field: 'weight', operator: 'lte', value: weightSlider.values()[1] });
        rangesliderFilters.push({ field: 'horsepower', operator: 'gte', value: horsepowerSlider.values()[0] });
        rangesliderFilters.push({ field: 'horsepower', operator: 'lte', value: horsepowerSlider.values()[1] });
        rangesliderFilters.push({ field: 'displacement', operator: 'gte', value: displacementSlider.values()[0] });
        rangesliderFilters.push({ field: 'displacement', operator: 'lte', value: displacementSlider.values()[1] });
        rangesliderFilters.push({ field: 'cylinders', operator: 'gte', value: cylindersSlider.values()[0] });
        rangesliderFilters.push({ field: 'cylinders', operator: 'lte', value: cylindersSlider.values()[1] });
        rangesliderFilters.push({ field: 'mpg', operator: 'gte', value: mpgSlider.values()[0] });
        rangesliderFilters.push({ field: 'mpg', operator: 'lte', value: mpgSlider.values()[1] });
        ...
        andfilter = {
            logic: "and",
            filters: rangesliderFilters
        };

        if (checkboxFilters.length > 0) {
            grid.dataSource.filter([orFilter, andfilter]);
        } else {
            grid.dataSource.filter(andfilter);
        };

        //grid.dataSource.filter([orFilter, andfilter]);
        console.log(checkboxFilters);
        console.log(rangesliderFilters);
        console.log(orFilter.logic);
    };
...
$("#grid").kendoGrid({
    ...
    detailTemplate: "<div>MPG: #: mpg #</div><div>Cylinders: #: cylinders #</div><div>Displacement: #: displacement #</div><div>Horsepower: #: horsepower #</div><div>Weight: #: weight #</div><div>Acceleration: #: acceleration #</div><div>Model-year: #: modelyear #</div><div>Make: #: make #</div>",
